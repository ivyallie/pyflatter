...
import Image, ImageDraw, ImageColor, random, timeit, subprocess, os, argparse
...
def closeGaps():
    ...
    if GetImage():
        print("Closing gaps...")
        subprocess.call(["convert "+args.infile+" -negate -morphology Close Square:3 /tmp/magick_output.png"], shell=True)
        subprocess.call(["mogrify -negate /tmp/magick_output.png"], shell=True)

    
def voronoi():
    """Create the file to be passed to the voronoi script, then call the script"""
    if not args.novor:
        if not args.scaling:
            im.save("/tmp/voronoi_input.png", "PNG")
            ...
            rsz.save("/tmp/voronoi_input.png", "PNG")
...
def CleanUp():
    ...
    try:
        os.remove("/tmp/magick_output.png")
    except OSError:
        pass
    
    try:
        os.remove("/tmp/voronoi_input.png")
    except OSError:
        pass
...
parser.add_argument('-s', '--scale', help='Scale the image by a given percentage to speed up voronoi processing',
                    type=validPercent, dest="scaling", metavar='PERCENTAGE')
...
if args.nogaps:
    ...
    closeGaps()
    im = Image.open("/tmp/magick_output.png").convert('RGBA')
    getAllPixels()
    voronoi()
